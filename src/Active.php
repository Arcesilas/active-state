...
class Active
{
    ...
    */
    protected function checkActiveDeeply($url, $method)
    {
        if (is_array($url)) {
            ...
            return call_user_func_array(array($this->request,$method), $urls)
                ? $this->activeValue
                : $this->inActiveValue;
        } else {
            return $this->request->is($url) || $this->request->is($url . '/*')
                ? $this->activeValue
                : $this->inActiveValue;
        }
    }
    ...
    */
    ...
    {
        if (is_array($url)) {
            return call_user_func_array(array($this->request,$method), $url)
                ? $this->activeValue
                : $this->inActiveValue;
        } else {
